...
class PassGBacklogTest extends TestCase
{
    ...
    public function test_admin_can_manage_blog_posts_and_categories(): void
    {
        ...
        $this->deleteJson('/api/v1/admin/cms/blog/posts/'.$post->id)->assertOk();
    }

    public function test_public_blog_supports_categories_and_pagination(): void
    {
    ...
    public function test_admin_can_manage_homepage_sliders_and_banners(): void
    {
        $admin = User::query()->where('email', 'superadmin@selloff.test')->firstOrFail();
        Sanctum::actingAs($admin);
        ...
        $this->assertSame($initialSliders + 1, Slider::query()->count());
        $this->assertSame($initialBanners + 1, HomepageBanner::query()->count());
    }

    public function test_products_index_supports_discovery_filters(): void
    {
    ...
    public function test_homepage_and_catalog_include_products_with_legacy_numeric_visibility(): void
    {
        $product = Product::query()->where('sku', 'DEMO-PHONE-1')->firstOrFail();
        ...
        $this->assertGreaterThan(0, $surfaceCount);
    }

    public function test_product_detail_supports_comments_feedback_and_reports(): void
    {
    ...
    public function test_admin_can_read_contact_messages_and_moderate_comments(): void
    {
        $admin = User::query()->where('email', 'superadmin@selloff.test')->firstOrFail();
        Sanctum::actingAs($admin);
    ...
    public function test_buyer_can_close_support_ticket(): void
    {
        $buyer = User::query()->where('email', 'buyer@selloff.test')->firstOrFail();
        Sanctum::actingAs($buyer);
