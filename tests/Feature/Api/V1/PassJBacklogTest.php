...
class PassJBacklogTest extends TestCase
{
    ...
    public function test_user_can_update_social_media_via_me_endpoint(): void
    {
        ...
        $this->assertSame('https://facebook.com/demo-shop', $profile->social_media_data['facebook']);
    }

    public function test_buyer_can_update_social_media_on_user_record(): void
    {
        $buyer = User::query()->where('email', 'buyer@selloff.test')->firstOrFail();
        Sanctum::actingAs($buyer);
        ...
        $buyer->refresh();
        $this->assertSame('https://instagram.com/demo-buyer', $buyer->social_media_data['instagram']);
    }

    public function test_public_vendor_reviews_are_paginated(): void
    {
    ...
    public function test_vendor_can_manage_feedback_inbox(): void
    {
        $vendor = User::query()->where('email', 'vendor@selloff.test')->firstOrFail();
        $buyer = User::query()->where('email', 'buyer@selloff.test')->firstOrFail();
    ...
    public function test_vendor_can_moderate_product_comments(): void
    {
        $vendor = User::query()->where('email', 'vendor@selloff.test')->firstOrFail();
    ...
    public function test_admin_can_read_and_update_feedback(): void
    {
    ...
    public function test_admin_can_manage_custom_fields_and_tags(): void
    {
        $admin = User::query()->where('email', 'superadmin@selloff.test')->firstOrFail();
    ...
    public function test_admin_can_view_earnings_reports_and_update_payout_settings(): void
    {
        $admin = User::query()->where('email', 'superadmin@selloff.test')->firstOrFail();
        Sanctum::actingAs($admin);

        $this->assertGreaterThan(0, VendorEarning::query()->count());
    ...
    public function test_admin_can_moderate_reviews_and_list_digital_sales(): void
    {
        $admin = User::query()->where('email', 'superadmin@selloff.test')->firstOrFail();
    ...
    public function test_public_and_admin_knowledge_base_crud(): void
    {
        $admin = User::query()->where('email', 'superadmin@selloff.test')->firstOrFail();
        Sanctum::actingAs($admin);
    ...
    public function test_admin_can_moderate_conversations(): void
    {
        $buyer = User::query()->where('email', 'buyer@selloff.test')->firstOrFail();
        $vendor = User::query()->where('email', 'vendor@selloff.test')->firstOrFail();
        $admin = User::query()->where('email', 'superadmin@selloff.test')->firstOrFail();
        ...
        $this->deleteJson('/api/v1/admin/messages/conversations/'.$conversation->id)
            ->assertOk();

        $this->assertDatabaseMissing('conversations', ['id' => $conversation->id]);
    }

    public function test_guest_can_checkout_with_bank_transfer(): void
    {
        $product = Product::query()->where('sku', 'DEMO-PHONE-1')->firstOrFail();
    ...
    public function test_vendor_can_purchase_membership_and_list_transactions(): void
    {
        $vendor = User::query()->where('email', 'vendor@selloff.test')->firstOrFail();
    ...
    public function test_admin_can_list_membership_transactions(): void
    {
        $admin = User::query()->where('email', 'superadmin@selloff.test')->firstOrFail();
        Sanctum::actingAs($admin);

        $this->assertGreaterThan(0, MembershipTransaction::query()->count());
    ...
    public function test_vendor_can_list_promotion_transactions(): void
    {
        $vendor = User::query()->where('email', 'vendor@selloff.test')->firstOrFail();
        Sanctum::actingAs($vendor);

        $this->assertGreaterThan(0, PromotionTransaction::query()->where('user_id', $vendor->id)->count());
    ...
    public function test_registration_queues_email_verification_and_token_verifies_email(): void
    {
        ...
        $this->assertGreaterThan(0, EmailJob::query()->where('to_email', 'passj6.verify@selloff.test')->count());
    ...
    public function test_user_can_update_location_preference_on_me_endpoint(): void
    {
        $buyer = User::query()->where('email', 'buyer@selloff.test')->firstOrFail();
    ...
    public function test_admin_can_update_grouped_platform_settings(): void
    {
        $admin = User::query()->where('email', 'superadmin@selloff.test')->firstOrFail();
        Sanctum::actingAs($admin);
