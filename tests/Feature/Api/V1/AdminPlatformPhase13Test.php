...
test('admin can bulk moderate blog comments', function () {
...
test('admin can manage platform preferences newsletter and affiliate program', function () {
    $admin = User::query()->where('email', 'superadmin@selloff.test')->firstOrFail();
    Sanctum::actingAs($admin);
    ...
    expect((int) ($program['commission_rate'] ?? 0))->toBe(8);
});

test('admin can delete abuse reports', function () {
    $admin = User::query()->where('email', 'superadmin@selloff.test')->firstOrFail();
    Sanctum::actingAs($admin);
