...
test('currency rate refresh updates exchange rates when converter configured', function () {
    ...
    expect(Currency::query()->where('code', 'USD')->value('exchange_rate'))->toBe('0.001200');
});

test('homepage category layout can sync featured and index order', function () {
    $admin = User::query()->where('email', 'superadmin@selloff.test')->firstOrFail();
    Sanctum::actingAs($admin);
    ...
    expect($firstFeatured->is_featured)->toBeTrue();
    expect((int) $firstFeatured->featured_order)->toBe(1);
});

test('granular permission allows homepage manager without full admin panel', function () {
...
test('platform cache preferences and abuse reports endpoints', function () {
    $admin = User::query()->where('email', 'superadmin@selloff.test')->firstOrFail();
    Sanctum::actingAs($admin);
...
test('payment gateway settings support wallet deposit depth', function () {
    $admin = User::query()->where('email', 'superadmin@selloff.test')->firstOrFail();
    Sanctum::actingAs($admin);
