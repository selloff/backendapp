...
class PassPhase10BacklogTest extends TestCase
{
    ...
    public function test_admin_can_mark_bank_transfer_order_paid_and_update_line_status(): void
    {
    ...
    public function test_admin_can_reject_payout_and_view_seller_account(): void
    {
        $admin = User::query()->where('email', 'superadmin@selloff.test')->firstOrFail();
    ...
    public function test_admin_can_manage_category_hierarchy(): void
    {
        $admin = User::query()->where('email', 'superadmin@selloff.test')->firstOrFail();
        Sanctum::actingAs($admin);
        ...
        $this->deleteJson("/api/v1/admin/categories/{$parentId}")
            ->assertStatus(422);
    ...
    public function test_admin_can_manage_location_cities(): void
    {
        $admin = User::query()->where('email', 'superadmin@selloff.test')->firstOrFail();
        Sanctum::actingAs($admin);
        ...
        $this->deleteJson("/api/v1/admin/locations/cities/{$cityId}")->assertOk();
    ...
    public function test_admin_can_create_user_with_roles(): void
    {
        ...
        $this->assertTrue($created->hasRole('vendor'));
    }
