...
class MembershipEntitlementsPass1Test extends TestCase
{
    ...
    public function test_admin_can_create_plan_with_structured_entitlements_and_category_limits(): void
    {
    ...
    public function test_admin_rejects_child_category_in_plan_limits(): void
    {
        $admin = User::query()->where('email', 'superadmin@selloff.test')->firstOrFail();
    ...
    public function test_membership_activation_stores_entitlement_snapshot_and_top_credits(): void
    {
        $admin = User::query()->where('email', 'superadmin@selloff.test')->firstOrFail();
        ...
        $this->assertSame(8, (int) $subscription->top_credits_remaining);
        $this->assertIsArray($subscription->entitlements_snapshot);
        $this->assertSame('VIP Snapshot Plan', $subscription->entitlements_snapshot['plan_title']);
        $this->assertSame(10.0, (float) $subscription->entitlements_snapshot['visibility_multiplier']);
        $this->assertSame(15, (int) $subscription->entitlements_snapshot['category_limits'][(string) $rootCategory->id]);
    }

    public function test_vendor_entitlements_endpoint_returns_usage_and_limits(): void
    {
        $vendor = User::query()->where('email', 'vendor@selloff.test')->firstOrFail();
    ...
    public function test_public_membership_catalog_includes_entitlements_block(): void
    {
